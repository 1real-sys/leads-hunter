...
2. **Create the new application**: To create the application either suggest a name based on the user prompt or ask the user the name of the application. Create the application with the following command:
   ...
   `npx @angular/cli@latest new <app-name> [list of flags based on the description of the app] --interactive=false --ai-config=open-ai-codex`

   _Important_: Use the valid `--ai-config` value for the active agent. For Codex, use `--ai-config=open-ai-codex`.
