...
A sprint **INFO-01** está em implementação em `features-pos-mvp/pesquisa-inteligente.md`, com **INFO-01.1, INFO-01.2 e INFO-01.3 concluídas em 12/09/2026**. Após o cliente HTTP simples receber apenas a exigência de JavaScript, foi aprovado o Chromium headless no servidor. `GooglePesquisaWebClient` monta consultas separadas para Instagram e site próprio com nome, categoria e localização; o navegador Playwright renderiza somente a busca pública do Google e o parser jsoup converte URL, título e resumo em DTOs internos.

Uma única instância do browser é reutilizada em thread dedicada, com contexto isolado por consulta, fila padrão de um item, timeout, limite de resposta, bloqueio de recursos pesados e destinos restritos a hosts do Google. O intervalo padrão entre navegações é de 15 segundos. Ao detectar CAPTCHA/tráfego incomum, o client entra em cooldown de uma hora e o lote encerra os acessos restantes; três falhas técnicas consecutivas também abrem o circuito do lote. Não há retry automático nem mecanismo de evasão. Fixtures cobrem resultado, ausência real, captcha e formato alterado. O smoke real abriu o Chromium, mas o Google classificou o IP atual como tráfego incomum e redirecionou para `/sorry`; o client reconheceu o bloqueio sem tentar contornar captcha ou expor HTML. O binário Chromium precisa ser instalado no host pelo CLI do Playwright e não faz parte do JAR.

`PesquisaWebInternaService` executa exatamente duas consultas por lead e classifica os conjuntos separadamente. URLs são canonicalizadas sem parâmetros de rastreamento; perfil do Instagram precisa ter um único segmento válido, e posts/reels/stories/áreas internas são recusados. Sites rejeitam redes sociais, mapas, diretórios, agregadores, marketplaces, avaliações e delivery, além de hosts locais, IPs, credenciais e portas não padrão. Nome, domínio/handle, categoria, município, endereço, telefone, CNPJ, razão social e `googlePlaceId` público compõem uma pontuação conservadora. Candidato abaixo do limiar, empate, homônimo sem confirmação ou CNPJ conflitante produz ausência. Nenhuma URL candidata é acessada.
...
Os 57 testes unitários direcionados da pesquisa e orquestração passaram, assim como os dois testes de integração JPA da INFO-01.3. O pacote executável foi gerado. A suíte backend completa executou 221 testes: 214 passaram, um smoke opt-in foi ignorado e permaneceram as mesmas três falhas e três erros preexistentes da blacklist e da carga CNPJ local; nenhum teste da pesquisa inteligente falhou.
