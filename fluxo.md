...
`PesquisaWebInternaService` executa duas consultas iniciais por lead e uma terceira de Instagram sem município apenas se necessário. Os resultados das consultas são avaliados em conjunto para ambos os tipos, sem perder homônimos ou conflitos anteriores. URLs são canonicalizadas sem parâmetros de rastreamento; Instagram exige perfil, recusando posts/reels/stories. Sites rejeitam redes sociais, diretórios, agregadores, marketplaces e destinos inseguros. Nome/handle iguais não bastam: exige-se evidência independente de município, endereço com número, telefone, CNPJ ou `googlePlaceId` público. A categoria e a razão social continuam como sinais de pontuação, não substitutos da confirmação. Nenhuma URL candidata é acessada.
...
O refinamento solicitado está implementado e a API responde às consultas reais. O aceite amplo da **INFO-01.7** permanece parcial: falta uma amostra rotulada maior de URLs realmente pertencentes aos estabelecimentos para medir precisão/cobertura. Capturas antigas por nome/handle não devem ser tratadas como identidade comprovada; não houve correção retroativa dos blocos já gravados. O scraping permanece desativado. Falhas técnicas não produzem ausência conclusiva nem alteram observações; candidatos sem confirmação são recusados.
